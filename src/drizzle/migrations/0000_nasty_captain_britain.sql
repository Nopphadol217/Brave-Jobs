...
CREATE TABLE "job_listings" (
	...
	"orgranizationId" varchar NOT NULL,
...
CREATE TABLE "organization_user_settings" (
	"userId" varchar NOT NULL,
	"organnizationId" varchar NOT NULL,
	"newApplicationEmailNotifications" boolean DEFAULT false NOT NULL,
	"miniumRating" integer,
	"createdAt" timestamp with time zone DEFAULT now() NOT NULL,
	"updatedAt" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "organization_user_settings_userId_organnizationId_pk" PRIMARY KEY("userId","organnizationId")
);
--> statement-breakpoint
ALTER TABLE "job_listings" ADD CONSTRAINT "job_listings_orgranizationId_organizations_id_fk" FOREIGN KEY ("orgranizationId") REFERENCES "public"."organizations"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "organization_user_settings" ADD CONSTRAINT "organization_user_settings_organnizationId_organizations_id_fk" FOREIGN KEY ("organnizationId") REFERENCES "public"."organizations"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
